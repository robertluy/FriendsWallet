...
async def main():
    ...
    tg_tag = await dbw.get_tg_tag_from_tg_id(1)
    tg_id_by_tag = await dbw.get_tg_id_from_tg_tag('@RealRoberL')
    tg_tag_by_tg_id = await dbw.get_tg_tag_from_tg_id('410403')
    wallet = await dbw.get_tg_id_wallet(1)

    print('unpaid:', unpaid)
    print('paid:', paid)
    print('debt of him:', debt_of_him)
    print('tg_tag from tg_id:', tg_tag)
    print('tg_id from tg_tag:', tg_id_by_tag)
